                ...
                commonUtil.httpGetPromise(
                    ...
                    .then((tx) => {
                        ...
                        if (tx.hasOwnProperty('error_code')) {
                            if (tx.error_code === 4) {
                                handleTransWithTimeout({'id': ele[0]});
...
function saveTrans(trans) {
    ...
    if (!commonUtil.objectCheck(trans, {'hash': 'string'})) {
        return;
    }
    ...
    if (dbId.length === 0) {
        ...
        db.run('update ' + tableLink +
            ' set time=?, nonce=? where tid=?',
            [txTime, txNonce, dbId[0].values[0][0]]);
        updateDB();
        ...
        .then((data) => {
            ...
            if (result.error_code == 0) {
                if (result.result && result.result.transactions) {
                    for (let j = 0;
                        j < result.result.transactions.length; j++) {
...
function handleTransWithTimeout(params) {
    let id = null;
    if (commonUtil.objectCheck(params, {'hash': 'string'})
    ...
    if (commonUtil.objectCheck(params, {'id': 'number'})) {
        id = params.id;
    }
    if (id) {
        db.exec(
                        ...
                        .transaction.operationsList.every((ele)=>{
                            switch (ele.type) {
                                case 1:
                                    ...
                                    break;
                                // case 3:
                                //     if (ele.hasOwnProperty('payment')
                                //         && ele.payment
                                //             .hasOwnProperty('destAddress')) {
                                //         if (ele.payment.destAddress == addr) {
                                //             saveTrans(trans);
                                //         }
                                //     }
                                //     break;
        ...
        for (let i = 0; i < trans.transaction.operations.length; i++) {
            if (trans.transaction.operations[i].type) {
                switch (trans.transaction.operations[i].type) {
                    ...
                    // case 3:
                    //     if (trans.transaction.operations[i]
                    //         .hasOwnProperty('payment')
                    //         && trans.transaction.operations[i].payment
                    //         .hasOwnProperty('dest_address')) {
                    //         if (trans.transaction.operations[i]
                    //             .payment.dest_address == addr) {
                    //             saveTrans(trans);
                    //         }
                    //     }
                    //     break;
