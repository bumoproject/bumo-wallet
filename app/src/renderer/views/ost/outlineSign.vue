<template>
  <div class="outline-sign-wrapper">
    <div class="unsigned-tx-str ost-card-box ost-card-box-background">
      <div class="ost-card-title ost-card-title-background">{{$t('ost.outlineSign.unsignedBlob.title')}}</div>
      <textarea @blur="handleBlobIptBlur" 
                class="unsigned-tx-str-ipt" 
                v-model="txBlob"
                :placeholder="$t('ost.outlineSign.unsignedBlob.placeholder')"
                name="" id="" cols="100" rows="5">
      </textarea>
    </div>
    <div class="unsigned-tx-str-parser ost-card-box clearfix">
      <div class="ost-card-title">{{$t('ost.outlineSign.parserDetail.title')}}</div>
      <div class="parser-wrapper">
        <div v-if="!showParser" class="default-no-data">
          <svg class="icon no-data-icon" aria-hidden="true">
            <use xlink:href="#icon-zanwushuju"></use>
          </svg>
          <div class="default-no-data-text">{{$t('ost.outlineSign.parserDetail.noData')}}</div>
        </div>
        <div v-else class="parser-content">
          <div class="ost-card-item ost-card-item-special">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('ost.outlineSign.parserDetail.srcAddr')}}</div>
            <div class="ost-card-item-content ost-card-item-content-special">{{txInfo.params.srcAddress}}</div>
          </div>
          <div class="ost-card-item ost-card-item-special">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('ost.outlineSign.parserDetail.destAddr')}}</div>
            <div class="ost-card-item-content ost-card-item-content-special">{{txInfo.params.destAddress}}</div>
          </div>
          <div class="ost-card-item ost-card-item-special">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('ost.outlineSign.parserDetail.txAmount')}} (BU)</div>
          ...
          <div class="ost-card-item ost-card-item-special ost-card-item-remarks">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('ost.outlineSign.parserDetail.note')}}</div>
            <!--  ? txInfo.params.note : $t('ost.outlineSign.parserDetail.noNote') -->
    ...
    <div v-if="showBlob" class="unsubmitted-tx-str ost-card-box">
      <div class="ost-card-title">{{$t('ost.outlineSign.unSubmittedBlob.title')}}</div>
      <div class="unsubmitted-tx-str-notice">{{$t('ost.outlineSign.unSubmittedBlob.notice')}}</div>
    ...
    <div class="ost-btn outline-sign-btn" 
         ...
         @click="handleOutlineSignClick">{{showBlob ? $t('ost.outlineSign.clearBtn') : $t('ost.outlineSign.signBtn')}}</div>
    <Modal
      :title="$t('ost.outlineSign.dialog.title')"
      ...
      <div class="play-pwd-wraper">
        <Form ref="playPwdData" :model="signTxData" :rules="playPwdRuleValidate">
          <FormItem prop="accountPwd">
            <Input v-model="signTxData.accountPwd" :placeholder="$t('ost.outlineSign.dialog.placeholder')" type="password"></Input>
          </FormItem>
        </Form>
      </div>
      <div slot="footer">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleOutlineSignClick () { // 签名或清空按钮的点击
...
<style scoped lang="less">
  ...
  .outline-sign-wrapper {
    padding: 20px 20px 0 20px;
  }
  .outline-sign-wrapper .unsigned-tx-str {
    height: 126px;
  }
  .outline-sign-wrapper .unsigned-tx-str-parser {
    padding-bottom: 20px;
    min-height: 188px;
  }
  .outline-sign-btn {
  ...
  .ivu-icon-ios-close-empty:after {
    content: "";
    position: absolute;
    top: 5px;
    right: 0px;
