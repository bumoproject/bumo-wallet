<template>
  <div v-if="!blockStatus" class="submit-tx-wrapper">
    <div class="unsubmitted-tx-str ost-card-box ost-card-box-background">
      <div class="ost-card-title ost-card-title-background">{{$t('unite.submitTx.unsubmittedBlob.title')}}</div>
      <textarea @blur="handleBlobIptBlur" 
                class="unsubmitted-tx-str-ipt" 
                v-model="txBlob"
                :placeholder="$t('unite.submitTx.unsubmittedBlob.placeholder')"
                autofocus name="" id="" cols="100" rows="5">
      </textarea>
    </div>
    <div class="unsubmitted-tx-str-analysis ost-card-box">
      <div class="ost-card-title">{{$t('unite.submitTx.parserDetail.title')}}</div>
      <div class="analysis-wrapper">
        <div v-if="noData" class="default-no-data">
          <svg class="icon no-data-icon" aria-hidden="true">
            <use xlink:href="#icon-zanwushuju"></use>
          </svg>
          <div class="default-no-data-text">{{$t('unite.submitTx.parserDetail.noData')}}</div>
        </div>
        <div v-else class="analysis-content">
          <div class="ost-card-item ost-card-item-special">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('unite.submitTx.parserDetail.srcAddr')}}</div>
            <div class="ost-card-item-content ost-card-item-content-special">{{txInfo.params.srcAddress}}</div>
          </div>
          <div class="ost-card-item ost-card-item-special">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('unite.submitTx.parserDetail.destAddr')}}</div>
            <div class="ost-card-item-content ost-card-item-content-special">{{txInfo.params.destAddress}}</div>
          </div>
          <div class="ost-card-item ost-card-item-special">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('unite.submitTx.parserDetail.txAmount')}} ({{txInfo.params.code || 'BU'}})</div>
          ...
          <div class="ost-card-item ost-card-item-special ost-card-item-remarks">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('unite.submitTx.parserDetail.note')}}</div>
            <!--  ? txInfo.params.note : $t('unite.submitTx.parserDetail.noNote') -->
          ...
          <div class="ost-card-item ost-card-item-special ost-card-item-many-members">
            <div class="ost-card-item-name ost-card-item-name-special">{{$t('unite.submitTx.parserDetail.signedUniteMember')}}</div>
            <div class="ost-card-item-content-special">
              <div class="ost-card-item-content" v-if="txInfo.signers.length === 0">{{$t('unite.submitTx.parserDetail.noSignedUniteMember')}}</div>
    ...
    <div class="ost-btn submit-tx-btn" 
         ...
         {{submitBtnText ? $t('unite.submitTx.signBtn') : $t('unite.submitTx.clearBtn')}}
    </div>
  </div>
  <div v-else class="sync-process-wrapper ostSpecial">
    <div class="sync-process-box">  
      ...
      <div class="sync-process-text">{{$t('unite.submitTx.noSyncBlock')}}</div>
    </div>  
  </div>
</template>
...
<style scoped lang="less">
  ...
  .submit-tx-wrapper .unsubmitted-tx-str {
    height: 126px;
  }
  ...
  .submit-tx-wrapper .unsubmitted-tx-str-analysis {
    padding-bottom: 20px;
    overflow: hidden;
  }
  .submit-tx-btn {
